...
cols = ['id', 'object_id', 'date_time', 'latitude', 'longitude', 'temperature', 'dewpoint', 'relative_humidity', 'precipitation', 'snow', 'wind_direction', 'wind_speed', 'wind_gust', 'air_pressure', 'sunshine', 'weather_code', 'fire']

augmented_data_df = pd.DataFrame(augmented_data, columns=cols)
augmented_data_df = augmented_data_df.drop('object_id', axis=1)
augmented_data_df = augmented_data_df.drop('id', axis=1)
